...
/**
 * A Map is a data structure to store data under key values, or just keys. For
 * each key there can be at most one datum that is associated to that key. Maps
 * provide methods to lookup data {@link #find(Object)}, to change the map
 * {@link #insert(Object, Object)}, {@link #delete(Object)}, to iteratate over
 * keys {@link #simpleiterator()} and over both keys and data
 * {@link #iterator()}. Moreover, there are possibilities to display a map
 * {@link #print()} and to store a map in an array {@link #array()}.
 * 
 * 
 * @author Rossmanith
 *
...
public interface Map<K, D> {
	 ...
	 * 
	 * @return The {@link Iterator} over all keys with associated data.
	 */
